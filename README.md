...
ACCESS_TOKEN="<GASのスクリプトプロパティで設定したトークン>"
PORT=3000
```

- `BOT_WEBHOOK_URL` : Google Apps Script のデプロイ URL
- `ACCESS_TOKEN` : GASのWebAppへのアクセス用のトークン（オプション）
  - 設定しない場合は認証なしでリクエストが処理されます
  - 設定した場合は、Webhookリクエスト時に`Authorization: Bearer `ヘッダーで送信

### 1-3. アクセストークンの設定（オプション）

セキュリティを強化するために、Google Apps Script（GAS）のアクセストークンを設定することができます。
...
1. GASアクセストークンの取得方法：
   ...
   ```
   - 実行ログに表示されたトークンをコピー
   - このトークンは一時的なものなので、定期的に更新が必要です

2. トークンの使用方法：
   - Webhookリクエスト時に`Authorization: Bearer <取得したトークン>`ヘッダーを付与
   - トークンが設定されていない場合は、認証なしでリクエストが処理されます

3. Web GUIでの設定方法：
   - ブラウザで`http://localhost:3000`にアクセス
   - 「設定」ボタンをクリック
   - 「アクセストークン」欄に取得したトークンを入力
   - 「設定を保存」をクリック
   - 設定は`.env`ファイルに自動的に保存されます

4. セキュリティ上の注意：
   - トークンは安全に管理してください
   - 本番環境では必ずトークンを設定することを推奨します
   - トークンは一時的なものなので、定期的に更新が必要です
   - トークンの有効期限は約1時間です

参考：[[Google Apps Script]認証が必要なウェブアプリケーションを外部から実行する](https://www.ka-net.org/blog/?p=12258)
